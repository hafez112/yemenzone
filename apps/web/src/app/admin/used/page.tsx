...
const CONDS: Record<string, string> = { 'like-new': '✨ كالجديد', 'used-good': '👍 جيد', 'used-fair': '🔧 مقبول' };
...
export default function AdminUsedPage() {
  ...
  const load = useCallback(async (s = status, query = q) => {
    setLoading(true);
    try { setData(await api(`/admin/used?status=${s}${query ? `&q=${encodeURIComponent(query)}` : ''}`)); }
  ...
  const act = async (id: string, st: 'active' | 'hidden') => {
    try {
      await api(`/admin/used/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status: st }) });
      toast(st === 'active' ? '✅ أُظهر الإعلان في السوق' : '🙈 أُخفي الإعلان عن الزوار');
      load();
    } catch (e: any) { toast(e.message || 'تعذّر', 'error'); }
  };

  const remove = async (id: string, title: string) => {
    if (!confirm(`حذف إعلان «${title}» نهائياً؟`)) return;
    try { await api(`/admin/used/${id}`, { method: 'DELETE' }); toast('🗑️ حُذف الإعلان'); load(); }
    catch (e: any) { toast(e.message || 'تعذّر', 'error'); }
  };

  const copyPage = (slug: string) => {
    navigator.clipboard.writeText(`${location.origin}/u/${slug}`).then(() => toast('📋 نُسخ رابط الإعلان')).catch(() => {});
  };
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <main className="flex-1 min-w-0 space-y-5" dir="rtl">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h1 className="text-2xl font-black">♻️ سوق المستعمل</h1>
              <p className="text-sm text-gray-500 mt-1">
                إعلانات الزوار في السوق — {c.total || 0} إعلان · 👁️ {(c.views || 0).toLocaleString()} مشاهدة
              </p>
            </div>
            <a href="/tools/used-market" target="_blank" className="px-4 py-2 rounded-xl bg-lime-600 text-white text-sm font-bold hover:bg-lime-500">👁️ معاينة السوق</a>
          </div>
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="flex gap-1 mr-auto">
              <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load(status, q)}
                placeholder="🔍 ابحث بعنوان الإعلان..." className="px-3 py-2.5 rounded-xl border border-gray-200 text-sm w-48 focus:outline-none focus:border-lime-400" />
              <button onClick={() => load(status, q)} className="px-3 py-2.5 rounded-xl bg-gray-900 text-white text-sm font-bold">بحث</button>
            </div>
          </div>

          {loading ? (
            <div className="grid place-items-center py-24"><div className="w-12 h-12 rounded-full border-4 border-gray-200 border-t-lime-500 animate-spin" /></div>
          ) : data.items.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center text-gray-400">
              <div className="text-5xl mb-3">📭</div><p className="font-bold">لا توجد إعلانات في هذه القائمة</p>
            </div>
          ) : (
                ...
                <div key={b.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  <div className="flex items-start gap-3 flex-wrap">
                    {Array.isArray(b.images) && b.images[0] ? (
                      ...
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-lime-500 to-emerald-600 grid place-items-center text-xl shrink-0">♻️</div>
                    )}
                    <div className="flex-1 min-w-52">
                      <p className="font-extrabold text-gray-900">
                        {b.title} <span className="text-lime-600">{Number(b.price).toLocaleString()} {cur(b.currency)}</span>
                        <span className="text-[10px] text-gray-400 font-normal mr-2">{new Date(b.createdAt).toLocaleDateString('ar-YE')}</span>
                      </p>
                      <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-lime-50 text-lime-700 border border-lime-100">{CATS[b.category] || b.category}</span>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-gray-50 text-gray-500 border border-gray-100">{CONDS[b.condition] || b.condition}</span>
                      </div>
                      {b.desc && <p className="text-sm text-gray-600 mt-1 leading-relaxed line-clamp-2">{b.desc}</p>}
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-gray-500 font-bold">
                        ...
                        <span>👁️ {b.views}</span>
                        <span className="text-gray-300" dir="ltr">/u/{b.slug}</span>
                      </div>
                    </div>
                    <div className="flex gap-2 shrink-0 flex-wrap">
                      <a href={`/u/${b.slug}`} target="_blank" className="px-4 py-2 rounded-xl bg-sky-600 text-white text-sm font-bold hover:bg-sky-500">👁️ الإعلان</a>
                      ...
                      <button onClick={() => remove(b.id, b.title)} className="w-9 h-9 grid place-items-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100">🗑️</button>
                    </div>
                  </div>
                </div>
              ))}
