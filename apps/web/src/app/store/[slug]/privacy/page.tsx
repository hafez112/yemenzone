...
import { privacyFor } from '@/lib/policies';


// سياسة الخصوصية — فريدة لكل نشاط، بمحتوى يناسب نوعه (فندق/إيجارات/خدمات/منتجات)
export default async function PrivacyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const policy = privacyFor(store.type?.kind || 'products', store.name, store.whatsapp);
        ...
        <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8">
          ...
          <div className="space-y-5 text-sm leading-relaxed text-gray-600">
            ...
            <section className="bg-gray-50 rounded-2xl p-4">
              ...
              <p className="text-xs text-gray-500">منصة يمن زون توفر البنية التقنية فقط ولا تتدخل في بيانات العملاء ولا تستخدمها لأي غرض تسويقي أو تجاري. كل نشاط مسؤول عن بيانات عملائه وفق هذه السياسة.</p>
            </section>
          </div>
